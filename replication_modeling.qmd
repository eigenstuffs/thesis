...
```{r}
baseline_emissions_FE <- lm(log(EmissionsPerCapita) ~ Ideology + factor(State) + factor(Year), data = panel)
...
for(i in 1:n_permutations) {
  ...
  model <- lm(log(EmissionsPerCapita) ~ Ideology + factor(State) + factor(Year), data=shuffled_data)
