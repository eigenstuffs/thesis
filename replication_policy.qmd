...
policies_dupe <- data.frame(lapply(policies_dupe, function(x) na.locf(x, na.rm = FALSE)))

policies_dupe <- data.frame(lapply(policies_dupe, function(x) {
  rev(na.locf(rev(x), na.rm = FALSE))
}))
...
policies_dupe <- merge(panel %>% dplyr::select(State, Year, Ideology), policies_dupe)
...
for (col in names(policies_dupe %>% dplyr::select(-State, -Year, -Ideology))) {
  if (is.factor(get(col, policies_dupe))) {
    formula <- as.formula(paste(as.factor(col), "~ Ideology + factor(State) + factor(Year)"))
    model <- suppressWarnings(glm(formula, data = policies_dupe, family = "binomial"))
    if (summary(model)$coefficients['Ideology', 'Pr(>|z|)'] < 0.05) {
      print("SIGNIFICANT:")
      print(formula)
      print(summary(model)$coefficients['Ideology', 'Estimate'])
...
for (col in names(policies_dupe %>% dplyr::select(-State, -Year, -Ideology))) {
  if (is.factor(get(col, x))) {
    formula <- as.formula(paste(col, "~ Ideology + factor(State) + factor(Year)"))
    model <- suppressWarnings(glm(formula, data = policies_dupe))
    if (summary(model)$coefficients['Ideology', 'Pr(>|t|)'] < 0.01) {
      #print("SIGNIFICANT:")
      #print(formula)
      #print(summary(model)$coefficients['Ideology', 'Pr(>|t|)'])
    } else {
      print("NOT SIGNIFICANT:")
      print(formula)
      print(summary(model)$coefficients['Ideology', 'Pr(>|t|)'])
