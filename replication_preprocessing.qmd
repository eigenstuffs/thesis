...
list <- list(ci, ei, gdpcurrent, pop, urban, total_empl, manu_empl, farm_empl)
```
...
p <- p %>% select(-Area, -RealGDP, -TotalEmployment, -FarmEmployment, -ManufacturingEmployment)
```
...
```{r}
write.csv(p, "panel_data_FEB11.csv", row.names = FALSE)
```
