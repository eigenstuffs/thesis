...
ideology <- data.frame(
  State = ideology$st,
  Year = ideology$year,
  House = ideology$hou_chamber,
  Senate = ideology$sen_chamber
)
ideology$House <- as.numeric(ideology$House)
ideology$Senate <- as.numeric(ideology$Senate)
ideology$Ideology <- as.numeric(ideology$Senate)

for (i in 1:nrow(ideology)) {
  if (!is.na(ideology[i,3]) && !is.na(ideology[i,4])) {
    ideology[i,5] <- as.numeric((ideology[i,3] + ideology[i,4])/2) # Average if we have both chambers
  }
}
...
ideology <- ideology %>% 
  ...
  mutate(
    ...
    IdeologyLag3 = dplyr::lag(Ideology, 3),
    IdeologyLag4 = dplyr::lag(Ideology, 4),
    IdeologyLag5 = dplyr::lag(Ideology, 5)
  ) %>% 
  ungroup()
...
```{r}
pv_with_states$GHI <- pv_with_states$global_horizontal_irradiance
wind_with_states$Speed120m <- wind_with_states$wind_speed_120meters
...
wind_with_states <- wind_with_states %>% group_by(NAME) %>%
                  summarise(Speed120m = mean(Speed120m, na.rm = TRUE),
                            )
...
colnames(pv_with_states) <- c("State", "GHI", "geometry")
colnames(wind_with_states) <- c("State", "Speed120m", "geometry")
