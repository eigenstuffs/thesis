...
list <- list(ci, ei, gdpcurrent, pop, farm, total, urban)
```
...
p$FarmProportion <- p$FarmWages/p$TotalWages
...
p <- p %>% select(-Area, -TotalWages, -FarmWages)
```
...
```{r}
write.csv(p, "panel_data_FEB6.csv", row.names = FALSE)
```
