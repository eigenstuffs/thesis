...
panel <- read.csv("p_JAN28.csv")[,2:33] # exlcuding ID column
...
**First Models with Fixed Effects**
...
```{r}
m5 <- lm(log(CI) ~ log(Chamber) + log(Weighted_GHI) + log(Weighted_Speed120m) + log(ManuUnion) + log(GDPpc) + log(OilProd) + log(GasProd) + log(CoalProd) + log(Population) + log(FarmWages)+ log(MiningWages), panel)
m6 <- lm(log(EI) ~ log(Chamber) + log(Weighted_GHI) + log(Weighted_Speed120m) + log(ManuUnion) + log(GDPpc) + log(OilProd) + log(GasProd) + log(CoalProd) + log(Population) + log(FarmWages) + log(MiningWages), panel)
m7 <- lm(log(CI_Lag1) ~ log(Chamber) + log(Weighted_GHI) + log(Weighted_Speed120m) + log(ManuUnion) + log(GDPpc) + log(OilProd) + log(GasProd) + log(CoalProd) + log(Population) + log(FarmWages)+ log(MiningWages), panel)
m8 <- lm(log(EI_Lag1) ~ log(Chamber) + log(Weighted_GHI) + log(Weighted_Speed120m) + log(ManuUnion) + log(GDPpc) + log(OilProd) + log(GasProd) + log(CoalProd) + log(Population) + log(FarmWages)+ log(MiningWages), panel)
...
plot_coefs(m5,m6,m7,m8, model.names = c("Carbon Intensity", "Energy Intensity",
                                             "CI_Lag1", "EI_Lag1"))
export_summs(m5,m6,m7,m8, model.names = c("Carbon Intensity", "Energy Intensity",
                                             "CI_Lag1", "EI_Lag1"),robust="HC3")

ggplot(panel%>%filter(Year==2020), aes(x=State, y=Weighted_GHI, color=Chamber)) + geom_point()
ggplot(panel%>%filter(Year==2020), aes(x=State, y=Weighted_Speed120m, color=Chamber)) + geom_point()
```
...
policies <- full_join(policies, panel %>% select(State, Year, Chamber, CI, EI))
...
for (i in 3:24) {
  if (is.factor(policies[,i])) {
    m <- multinom(policies[,i] ~ Chamber + factor(State) + factor(Year), data = policies)
    sm <- summary(m)
...
policies_sig <- policies %>% select(Year, State, Chamber, CI, EI, environment_ca_car_emissions_standards_21
...
for (i in 1:ncol(policies)) {
  if (is.factor(policies[,i])) {
    policies_temp <- policies %>% select(State, Year, Chamber) %>% mutate(Variable = policies[,i])
    policies_temp <- na.omit(policies_temp)
    suppressWarnings(print(ggplot(policies_temp, aes(x=Chamber, y=Variable, color=Year)) +
      geom_jitter() + theme_bw() + labs(title = colnames(policies)[i]) + theme(
        title = element_text(face = "bold", color = "black"),
        axis.title = element_text(face = "bold", color = "black"),
        panel.grid.major.x = element_blank()) + geom_vline(xintercept = median(policies_temp$Chamber, na.rm = TRUE), linetype = "dashed", color = "blue") + xlim(c(min(policies_temp$Chamber, na.rm = TRUE), max(policies_temp$Chamber, na.rm = TRUE))))
  )}
}

for (i in 1:ncol(policies)) {
  if (is.factor(policies[,i])) {
    policies_temp <- policies %>% select(State, Year, Chamber) %>% mutate(Variable = policies[,i]) %>% filter(Year >= 2005 & Year <= 2008)
    policies_temp <- na.omit(policies_temp)
    suppressWarnings(print(ggplot(policies_temp, aes(x=Chamber, y=Variable, color=Year)) +
      geom_jitter() + scale_color_gradient(low = "pink", high = "red")  + theme_bw() + labs(title = colnames(policies)[i]) + theme(
        title = element_text(face = "bold", color = "black"),
        axis.title = element_text(face = "bold", color = "black"),
        panel.grid.major.x = element_blank()) + geom_vline(xintercept = median(policies_temp$Chamber, na.rm = TRUE), linetype = "dashed", color = "black") + xlim(c(min(policies_temp$Chamber, na.rm = TRUE), max(policies_temp$Chamber, na.rm = TRUE))))
  )}
}

for (i in 1:ncol(policies_sig)) {
  if (is.factor(policies_sig[,i])) {
    policies_temp <- policies_sig %>% select(State, Year, Chamber) %>% mutate(Variable = policies_sig[,i])
    policies_temp <- na.omit(policies_temp)
    suppressWarnings(print(ggplot(policies_temp, aes(x=Chamber, y=Variable, color=Year)) +
...
for (i in 1:ncol(policies_sig)) {
  if (is.factor(policies_sig[,i])) {
    policies_temp <- policies_sig %>% select(State, Year, Chamber, CI, EI) %>% mutate(Variable = policies_sig[,i])
    policies_temp <- na.omit(policies_temp)
    print(ggplot(policies_temp, aes(x=Variable, y=CI, color=Chamber)) +
    ...
    print(ggplot(policies_temp, aes(x=Variable, y=EI, color=Chamber)) +
...
**Models with Wind and Solar Data**

```{r}
colnames(panel)
summary(
  lm(data = panel, formula = log(CI) ~ log(Weighted_GHI) + log(Weighted_Speed120m) + log(Chamber) + log(Population) + log(GDPCurrent) + factor(Year)))

ggplot(data=panel%>%filter(Year == 2019), aes(x=Solar_Irradiance, y=CI)) + geom_point(aes(fill=Chamber<10.7)) + geom_smooth(data = subset(panel, Chamber < 10.7 & Year == 2019), 
              aes(x = Solar_Irradiance, y = CI, color = "Chamber < 10.7"), 
              method = "lm", se = FALSE) +
  geom_smooth(data = subset(panel, Chamber >= 10.7 & Year == 2019), 
              aes(x = Solar_Irradiance, y = CI, color = "Chamber >= 10.7"), 
              method = "lm", se = FALSE)

ggplot(panel%>%filter(Year==2018), aes(x=Renewables,y=Weighted_Speed120m)) + geom_point()
```
...
```{r}
library("fastDummies")
colnames(policies)[1:2] <- c("State", "Year")
names(policies)[3:24] <- paste0("FACTOR_", names(policies)[3:24])
policies_join <- left_join(policies, panel%>%select(State,Year,Chamber,ManuUnion,OilProd,GasProd, CoalProd), by = c("State", "Year"))
policies_join[3:24] <- lapply(policies_join[3:24], as.factor)

sum(is.na(policies_join))

str(policies_join)
policies_daisy <- daisy(x=policies_join%>%select(-State,-Year))
h <- hclust(policies_daisy)
plot(h)
```
...
joined_first_year <- left_join(first_year_adopted, panel %>% select(State, Year, Chamber), by=c("State","Year"))

str(joined_first_year)
joined_first_year$State <- as.factor(joined_first_year$State)
joined_first_year$Year <- as.factor(joined_first_year$Year)

first_year_daisy <- daisy(x=joined_first_year)

average_H <- hclust(first_year_daisy, "average")

plot(average_H, labels=FALSE, main="Average")

clusters <- cutree(average_H, k = 5)

joined_first_year$cluster <- clusters

table(joined_first_year$cluster)

view(joined_first_year %>% filter(cluster == 1))
```
